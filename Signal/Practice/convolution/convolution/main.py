...
def make_signal(start_time, end_time, values):
    # BUG: values.copy() on a plain Python list gives back a plain list,
    # but every DiscreteSignal method (multiply, nonzero_samples, plot,
    # ...) assumes self.values is a NumPy array. This will crash the
    # first time it hits array-only behavior — e.g. multiply() does
    # `scalar * self.values`, which raises TypeError for a float scalar
    # against a list (or silently repeats the list for an int scalar).
    # Fix: sig.values = np.array(values, dtype=float)
    sig=DiscreteSignal(start_time,end_time)
    sig.values=np.array(values,dtype=float)
    return sig
...
def signal_from_samples(start_time, end_time, samples):
    ...
    for t in range(start_time, end_time + 1):
        if isinstance(samples, dict) and t in samples:
            sig.set_value_at_time(t, samples[t])
        elif isinstance(samples, (list, np.ndarray)) and 0 <= (t - start_time) < len(samples):
            sig.set_value_at_time(t, samples[t - start_time])
    return sig
...
def max_absolute_difference(first_signal, second_signal):
    ...
    # return float(np.max(np.abs(first_signal.values - second_signal.values)))
    max_diff = 0.0
    
    for t in first_signal.times():
        val1 = first_signal.get_value_at_time(t)
        val2 = second_signal.get_value_at_time(t)

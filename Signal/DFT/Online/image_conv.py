...
def linear_shape(image_shape, kernel_shape):
    """Shape of the full linear convolution: (H + kh - 1, W + kw - 1)."""
    return (image_shape[0] + kernel_shape[0] - 1,
            image_shape[1] + kernel_shape[1] - 1)


def transform_shape(image_shape, kernel_shape, engine):
    """
    Transform size for a linear convolution with ``engine``: the
    linear-convolution size, rounded up per axis when radix-2 is in use.
    """
...
def inverse_2d(spectrum, engine):
    ...
    columns = np.array([engine.inverse(column) for column in rows.T])
    return columns.T
...
def convolve_image(image, kernel, engine, circular=False):
    ...
    planes = [convolve_plane(image[:, :, i], kernel, engine, circular=circular)
...
def build_kernel(kernel_name, param):
    """Build the requested kernel with image_utils.make_kernel."""
    if kernel_name == "bokeh":
        return make_kernel("bokeh", radius=float(param))
    if kernel_name == "gaussian":
        return make_kernel("gaussian", size=int(param))
    if kernel_name == "box":
        return make_kernel("box", size=int(param))
    if kernel_name == "motion":
        return make_kernel("motion", length=int(param), angle=30.0)
...
def run_single(path, kernel_name, param, engine_name, out_dir, gray=False):
    ...
    kernel = build_kernel(kernel_name, param)
    ...
    verdict = "MATCH" if error < VERIFY_TOLERANCE else "MISMATCH"
    ...
    linear = linear_shape((H, W), kernel.shape)
    padded = transform_shape((H, W), kernel.shape, engine)
    write_report(os.path.join(out_dir, "report.txt"), [
        "Task B -- 2D convolution through the frequency domain",
        "%-20s: %s  (%d x %d, %s)" % ("image", path, H, W,
                                      "gray" if image.ndim == 2 else "RGB"),
        "%-20s: %s  (%d x %d)" % ("kernel", kernel_name,
                                  kernel.shape[0], kernel.shape[1]),
        "%-20s: %s" % ("engine", engine_name),
        "%-20s: %d x %d" % ("linear-conv size", linear[0], linear[1]),
        "%-20s: %d x %d" % ("transform size", padded[0], padded[1]),
        "max |spectral - direct| on %dx%d crop : %.3e"
        % (VERIFY_CROP, VERIFY_CROP, error),
        "%-20s: %s" % ("verification", verdict),
    ])

    print("%s -> %s  (%s %dx%d, %s, transform %dx%d, %s)"
          % (path, out_dir, kernel_name, kernel.shape[0], kernel.shape[1],
             engine_name, padded[0], padded[1], verdict))
    if verdict != "MATCH":
        raise SystemExit("VERIFICATION FAILED: max error %.3e" % error)

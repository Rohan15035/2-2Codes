...
def to_limbs(text, base_digits=BASE_DIGITS):
    ...
    if not digits.isdigit():
        raise ValueError("%r is not a decimal integer" % (text,))
...
def from_limbs(sign, limbs, base_digits=BASE_DIGITS):
    ...
    """
    base = 10 ** base_digits
    values = [int(v) for v in np.asarray(limbs).ravel()]
...
def multiply_schoolbook(a, b):
    ...
    coefficients = np.zeros(a.size + b.size - 1, dtype=np.int64)
...
def multiply(text_a, text_b, method):
    ...
    if method == "schoolbook":
        ...
        N = coefficients.size
    elif method in ENGINES:
        coefficients, N = multiply_transform(limbs_a, limbs_b, ENGINES[method]())
    else:
        raise ValueError("unknown method: %r" % (method,))

    product = from_limbs(sign_a * sign_b, coefficients)
...
def run_single(path, method, out_dir):
    ...
    write_report(os.path.join(out_dir, "report.txt"), [
        ...
        "%-20s: %s" % ("input file", path),
        "%-20s: %s" % ("method", method),
        "%-20s: %d / %d" % ("digits of A / B", digits_a, digits_b),
        "%-20s: 10^%d" % ("base", BASE_DIGITS),
        "%-20s: %d / %d" % ("limbs of A / B", limbs_a.size, limbs_b.size),
        "%-20s: %d" % ("transform length N", N),
        "%-20s: %d" % ("digits of product", digits_product),
        "%-20s: %s" % ("verification", verdict),
    ])

    print("%s -> %s  (%s, N = %d, %d digits, %s)"
          % (path, out_dir, method, N, digits_product, verdict))
    if verdict != "MATCH":

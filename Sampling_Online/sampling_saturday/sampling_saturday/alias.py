...
def lowest_alias_pair(f, fs):
    ...
    Sampling at fs maps every frequency f + k*fs and -f + k*fs onto the
    same sample set.  With 0 < f < fs/2 the candidates above zero are
    fs - f, fs + f, 2*fs - f, ...  and the smallest of those is fs - f.
    """
    return fs - f
